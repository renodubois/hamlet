...
- **Playwright** E2E in `e2e/`. `playwright.config.ts` spawns both `cargo run` (server) and `npm run dev` (client) via `webServer`. Tests rely on the server's seeded dev users (`baipas` / `password` and `teo` / `password`) and the auto-created `general` and `voice` channels. The server uses in-memory SQLite, so each run starts fresh.
