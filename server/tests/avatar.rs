...
async fn test_upload_avatar_sets_url_and_writes_file() {
    ...
    assert!(url.starts_with(&format!("/uploads/avatars/{}.webp?v=", alice.user_id)));

    let written = uploads_dir
        .join("avatars")
        .join(format!("{}.webp", alice.user_id));
...
// Sanity: the credential + session joins still work after the user entity
// picked up new columns. Lightweight regression guard.
#[actix_web::test]
