...
`it` is the source of truth for issue files. Issue files are always stored in the main repository at `/home/reno/projects/hamlet/issues`, not in any script-created worktree. Do not hand-write, rename, move, archive, or directly edit issue Markdown files unless the user explicitly asks. Run all tracker reads and writes against the main repository issue directory, even when implementation work happens in another worktree. The only tracker mutation this skill normally performs is marking finished issues completed with `it status <id> completed --dir /home/reno/projects/hamlet/issues`.
...
Run these from the main repository root (`/home/reno/projects/hamlet`), or use the absolute main issue directory from any cwd. Never point `it` at an issue worktree's `issues/` directory:

```bash
it list --dir /home/reno/projects/hamlet/issues
it list --dir /home/reno/projects/hamlet/issues --tag <tag>
it list --dir /home/reno/projects/hamlet/issues --tag <tag> --status in-progress
it list --dir /home/reno/projects/hamlet/issues --tag <tag> --status completed
it show <id> --dir /home/reno/projects/hamlet/issues
it status <id> completed --dir /home/reno/projects/hamlet/issues
```
...
Every AFK issue must be implemented in its own script-created worktree, never directly in the original repository worktree. Issue tracker state remains pinned to `/home/reno/projects/hamlet/issues`; do not copy, sync, mutate, or rely on issue files inside worktrees.

- Treat the original repository as the integration worktree and record its current branch/ref before launching workers. This is the "main working branch" for the run, even if it is not named `main`.
- Use `scripts/create-worktree.sh` from the original repository whenever creating or refreshing an issue worktree. Do not hand-create worktrees with `git worktree add` for this workflow.
...
- If a different root is explicitly needed, set `HAMLET_WORKTREE_ROOT`, but keep all issue worktrees grouped under one dedicated worktree directory rather than as siblings of the main repo. If a different port DB is explicitly needed, set `HAMLET_WORKTREE_PORT_DB`; otherwise use the script default.
...
- Never share one worktree between simultaneous issues. A worktree may be reused across issues only after the previous issue has been validated, committed, merged back to the main working branch, and the worktree has been hard-reset/cleaned back to the latest main working branch.
...
- Workers do not merge or update tracker status. The parent agent validates the worktree, creates an implementation commit there, merges that worktree branch into the main working branch, then marks the issue complete against `/home/reno/projects/hamlet/issues` from the main repository.
- If you remove a worktree after merging, release its reserved ports with `scripts/create-worktree.sh --release-port-allocation <worktree-dir>`. If you keep the worktree for reuse, keep its allocation reserved.
...
2. Use `it list --dir /home/reno/projects/hamlet/issues --tag <candidate>` to find the matching issue set.
...
For every issue in the selected tag, run `it show <id> --dir /home/reno/projects/hamlet/issues` and capture:
...
- If a blocker is outside the selected project, inspect it with `it show <id> --dir /home/reno/projects/hamlet/issues`; it must already be completed before the dependent issue can run.
...
1. In the original repository, run `git status --short`. Because this workflow commits and merges issue branches, the integration worktree should be clean except for changes this workflow is intentionally about to commit. If there are unrelated user changes you might overwrite or accidentally commit, ask before continuing.
2. Record the main working branch/ref from the original repository. If the repository is in a detached HEAD state or the branch/ref is ambiguous, ask before continuing.
3. For each ready issue selected for the batch, derive a unique branch name such as `issue/<id>-<kebab-title>` and run `scripts/create-worktree.sh --base-ref <main-working-branch> <issue-branch>` from the original repository root. Capture the reported absolute worktree path and assigned ports.
4. If `scripts/create-worktree.sh` reports that no free port slot is available, run `scripts/create-worktree.sh --list-port-allocations` and `scripts/create-worktree.sh --prune-port-allocations` to inspect/clean stale allocations when safe. If the failure remains ambiguous or would require releasing a live worktree's ports, ask the user before continuing.
...
Issue context:
<paste the full `it show <id> --dir /home/reno/projects/hamlet/issues` output or a faithful summary including acceptance criteria and blockers>
...
- Do not edit files in the original repository worktree.
- Issue tracker files live only in `/home/reno/projects/hamlet/issues`; do not use or edit worktree issue directories.
- Do not commit, stash, reset, checkout branches, rebase, push, merge, or edit issue tracker files.
- Do not mark the issue completed; the parent agent will validate, commit and merge the implementation, then update tracker status in `/home/reno/projects/hamlet/issues` from the main repository.
...
5. Create an implementation commit in the worktree. Do not include tracker files or issue status changes in the worktree commit. Use a clear message such as `Implement issue #<id>: <title>`.
6. Merge the worktree branch back into the main working branch in the original repository. Prefer a normal merge that preserves the issue commit. If the merge conflicts, resolve the conflict in the original repository, rerun affected checks, and continue only when the merge is sound.
7. Only after validation and merge, mark the issue completed from the main repository against the main issue directory:

   ```bash
   cd /home/reno/projects/hamlet
   it status <id> completed --dir /home/reno/projects/hamlet/issues
   ```

8. If the tracker status update changes tracked files, commit that update on the main working branch. Do not make tracker commits from an issue worktree.
9. Re-run `it show <id> --dir /home/reno/projects/hamlet/issues` or `it list --dir /home/reno/projects/hamlet/issues --tag <project-tag>` from the main repository to verify the completed status arrived on the main working branch.
10. Do not schedule dependent issues until their blockers have been validated, committed, merged, and verified completed in `/home/reno/projects/hamlet/issues` on the main working branch.
11. After a worktree has been merged, it may be reset/cleaned back to the latest main working branch and reused for a later issue batch. If you remove the worktree instead, run `scripts/create-worktree.sh --release-port-allocation <worktree-dir>` from the original repository to release its ports.
...
1. Run the appropriate final project checks for all touched sides from the main working branch. Use the repo's aggregate check script if practical.
2. Re-run `it list --dir /home/reno/projects/hamlet/issues --tag <project-tag>` to verify statuses.
3. Report concisely:
   - project tag
   - completed issues
...
Do not claim an issue is complete unless its tracker status is `completed` in `/home/reno/projects/hamlet/issues` on the main working branch and its acceptance criteria were verified.
